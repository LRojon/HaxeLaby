...
class Laby
{
	...
	public function new(screenWidth = 1280, screenHeight = 720, ?tileSize:Int)
	{
		if (tileSize != null)
		{
			this.width = Math.floor(screenWidth / tileSize);
			this.height = Math.floor(screenHeight / tileSize);
			this.tile = tileSize;
		}
		else
		{
			this.tile = findPGCD(screenWidth, screenHeight);
			this.width = Math.floor(screenWidth / this.tile);
			this.height = Math.floor(screenHeight / this.tile);
		}
